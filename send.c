...
/* $Id: send.c,v 1.2p 2019/11/21 10:11:56 antirez Exp $ */
...
static void select_next_random_source_pool(void)
{
	unsigned char ra[4];
	char a[4], b[4], c[4], d[4];

	if (sscanf(sourceaddrpool, "%4[^.].%4[^.].%4[^.].%4[^.]", a, b, c, d) != 4)
	{
		fprintf(stderr,
			"wrong --rand-source-pool IP address pool, correct examples:\n"
			"  x.x.x.x (becomes like --rand-source), 192,168.x.x, 128.x.x.255\n"
			"you typed: %s\n", sourceaddrpool);
		exit(1);
	}
	a[3] = b[3] = c[3] = d[3] = '\0';
	...
	if (opt_debug)
		printf("DEBUG: the source address is %u.%u.%u.%u (pool: %s)\n",
		    ra[0], ra[1], ra[2], ra[3], sourceaddrpool);
}
...
void send_packet (int signal_id)
{
	...
	} else if(opt_rand_source_pool) {
		select_next_random_source_pool();
	}
